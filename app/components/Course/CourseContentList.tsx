...
const CourseContentList: FC<Props> = ({ data, activeVideo, setActiveVideo, isDemo }) => {
  ...
  const videoSections: string[] = Array.from(new Set<string>(data?.map((item: VideoItem) => item.videoSection)));
  ...
  return (
    <div className={`mt-[15px] w-full ${!isDemo && "my-[15px] sticky top-24 left-0 z-30"}`}>
      {videoSections.map((section: string) => {
        ...
        const sectionVideos: VideoItem[] = data.filter((item: VideoItem) => item.videoSection === section);
                  ...
                  return (
                    ...
                    >
                      <div className='flex items-start'>
                        <div>
                          <MdOutlineOndemandVideo size={25} className='mr-2' color='#c1dada' />
                        </div>
